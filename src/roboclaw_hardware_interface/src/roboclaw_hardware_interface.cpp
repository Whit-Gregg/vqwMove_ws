...
namespace roboclaw_hardware_interface
{
  ...
  CallbackReturn RoboClawHardwareInterface::on_init(const HardwareInfo &hardware_info)
  {
    ...
    catch (const std::out_of_range &)
    {
      ...
      RCLCPP_ERROR(rclcpp::get_logger("RoboclawHardwareInterface"), "RoboClawHardwareInterface::on_init() 'serial_port' must be defined as a hardware parameter." );
    ...
    {
      //std::cerr << "RoboClawHardwareInterface::on_init() " << e.what() << std::endl;
    ...
    // interface_->write(  roboclaw_serial::FirmwareVersion()  );
    ...
    {
      //std::cerr << "RoboClawHardwareInterface::on_init() " << e.what() << std::endl;
      RCLCPP_ERROR(rclcpp::get_logger("RoboclawHardwareInterface"), "RoboClawHardwareInterface::on_init() parse_roboclaw_configuration, exception--> %s", e.what() );
      return CallbackReturn::ERROR;
    }
    ...
    try
    {
      ...
      {
        roboclaw.read_firmware_version();
      }
    }
    catch (const std::exception &e)
    {
      //std::cerr << "roboclaw.read_firmware_version  " << e.what() << std::endl;
      RCLCPP_ERROR(rclcpp::get_logger("RoboclawHardwareInterface"), "RoboClawHardwareInterface::on_init() roboclaw.read_firmware_version(), exception--> %s", e.what() );
      return CallbackReturn::ERROR;
    }
    ...
    for (auto joint : hardware_info.joints)
    {
      ...
      if (!roboclaw_config[roboclaw_address][motor_type])
      {
        ...
        roboclaw_config[roboclaw_address][motor_type] =
            std::make_shared<MotorJoint>(joint.name, qppr, wheel_diameter_meters);
      }
      else
      {
        throw std::runtime_error(
