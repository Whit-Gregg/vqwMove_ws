...
namespace bno08x_hardware_interface
{
    ...
    //-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~
    /// Initialization of the hardware interface from data parsed from the robot's
    /// URDF.
    /**
    ...
    {
        if (SensorInterface::on_init(hardware_info) != CallbackReturn::SUCCESS) { return CallbackReturn::ERROR; }
        //===================================================================================================
    ...
    // // //     // {
    // // //     //   sensor_state_interfaces_.insert(std::make_pair(description.get_name(),
    // // //     //   description));
    // // //     // }
    // // // }
    ...
    //-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~-=+~
    /**
     * Default implementation for exporting the StateInterfaces. The StateInterfaces
     * are created according to the InterfaceDescription. The memory accessed by the
     * controllers and hardware is assigned here and resides in the
     * sensor_interface.
     *
    ...
    std::vector<hardware_interface::StateInterface::ConstSharedPtr> Bno08xHardwareInterface::on_export_state_interfaces()
    {
        ...
        // RCLCPP_INFO(rclcpp::get_logger("bno08x_hardware_interface"),
        // "Bno08xHardwareInterface::on_export_state_interfaces()
        // pOrientation_X->get_name='%s'  get_interface_name='%s'
        // get_prefix_name='%s'",
        //             pOrientation_X->get_name().c_str(),
        //             pOrientation_X->get_interface_name().c_str(),
        //             pOrientation_X->get_prefix_name().c_str());
        ...
        if (elap_since_IMU_count_display > elap_since_IMU_count_display_span)
            {
                ...
                RCLCPP_INFO(rclcpp::get_logger("bno08x_hardware_interface"), "Bno08xHardwareInterface::read() "
                                                                             "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
                RCLCPP_INFO(rclcpp::get_logger("bno08x_hardware_interface"),
                            "Bno08xHardwareInterface::read()  count_of       Accel=%7ld    "
                            "%7.1f per sec.",
                            count_of_Accel, Accel_rate);
                RCLCPP_INFO(rclcpp::get_logger("bno08x_hardware_interface"),
                            "Bno08xHardwareInterface::read()  count_of        Gyro=%7ld    "
                            "%7.1f per sec.",
                            count_of_Gyro, Gyro_rate);
                RCLCPP_INFO(rclcpp::get_logger("bno08x_hardware_interface"),
                            "Bno08xHardwareInterface::read()  count_of         Mag=%7ld    "
                            "%7.1f per sec.",
                            count_of_Mag, Mag_rate);
                RCLCPP_INFO(rclcpp::get_logger("bno08x_hardware_interface"),
                            "Bno08xHardwareInterface::read()  count_of    LinAccel=%7ld    "
                            "%7.1f per sec.",
                            count_of_LinAccel, LinAccel_rate);
                RCLCPP_INFO(rclcpp::get_logger("bno08x_hardware_interface"),
                            "Bno08xHardwareInterface::read()  count_of Orientation=%7ld    "
                            "%7.1f per sec.",
                            count_of_Orientation, Orientation_rate);
                ...
                elap_since_IMU_count_display_span += elap_since_IMU_count_display_span / 3;
            }
    ...
    hardware_interface::CallbackReturn Bno08xHardwareInterface::on_configure([[maybe_unused]] const rclcpp_lifecycle::State &previous_state)
    {
        ...
        pChannelROS->addCallback<vqwPipe_chan_IMU_data_msg>([this](const vqwPipe_chan_IMU_data_msg *pMsg) { this->on_vqwPipe_chan_IMU_data_msg(pMsg); });

        pipe_rouge_data_buffer.set_Callback([this](const uint8_t *pData, int dataSize) { this->on_RougeData_line(pData, dataSize); });
        pPipe->set_RougeData_callback([this](const uint8_t *pData, int dataSize) { this->pipe_rouge_data_buffer.write(pData, dataSize); });
    ...
    hardware_interface::CallbackReturn Bno08xHardwareInterface::on_activate([[maybe_unused]] const rclcpp_lifecycle::State &previous_state)
    {
        ...
        if (!isOK)
            {
                RCLCPP_ERROR(rclcpp::get_logger("bno08x_hardware_interface"),
                             "Bno08xHardwareInterface::on_activate()  "
                             "pPipeDriver->connect(\'%s\', %d) failed !!!!! errno=%d  %s",
                             serial_port_name.c_str(), serial_port_speed, errno, strerror(errno));
                return CallbackReturn::ERROR;
            }
        RCLCPP_INFO(rclcpp::get_logger("bno08x_hardware_interface"),
                    "Bno08xHardwareInterface::on_activate()  "
                    "pPipeDriver->connect(\'%s\', %d) is OK.",
                    serial_port_name.c_str(), serial_port_speed);
    ...
    void Bno08xHardwareInterface::on_vqwPipe_chan_IMU_data_msg(const vqwPipe_chan_IMU_data_msg *pData)
    {
        if (count_of_IMU_data_msg == 0)
            {
                RCLCPP_INFO(rclcpp::get_logger("bno08x_hardware_interface"), "Bno08xHardwareInterface::on_vqwPipe_chan_IMU_data_msg() "
                                                                             "********************************* Yahh!, I got a IMU Data "
                                                                             "Message :-)");
            }
        ...
        switch (pData->data_type_code)
            {
                case vqwPipe_chan_IMU_data_msg::data_type_code_Accel:
                    // RCLCPP_INFO(rclcpp::get_logger("Bno08xHardwareInterface"), "Accel:
                    // X=%.3f, Y=%.3f, Z=%.3f", pData->X, pData->Y, pData->Z);
                    count_of_Accel++;
                    break;
                case vqwPipe_chan_IMU_data_msg::data_type_code_Gyro:
                    ...
                    // RCLCPP_INFO(rclcpp::get_logger("Bno08xHardwareInterface"), "Gyro: X=%.3f,
                    // Y=%.3f, Z=%.3f", pData->X, pData->Y, pData->Z);
                    break;
                case vqwPipe_chan_IMU_data_msg::data_type_code_Mag:
                    // RCLCPP_INFO(rclcpp::get_logger("Bno08xHardwareInterface"), "Mag: X=%.3f,
                    // Y=%.3f, Z=%.3f", pData->X, pData->Y, pData->Z);
                    count_of_Mag++;
                    break;
                case vqwPipe_chan_IMU_data_msg::data_type_code_LinAccel:
                    ...
                    // RCLCPP_INFO(rclcpp::get_logger("Bno08xHardwareInterface"), "LinAccel:
                    // X=%.3f, Y=%.3f, Z=%.3f", pData->X, pData->Y, pData->Z);
                    break;
                case vqwPipe_chan_IMU_data_msg::data_type_code_Orientation:
                    ...
                    // RCLCPP_INFO(rclcpp::get_logger("Bno08xHardwareInterface"), "Orientation:
                    // X=%.3f, Y=%.3f, Z=%.3f, W=%.3f", pData->X, pData->Y, pData->Z, pData->W);
                    break;
                default:
                    // RCLCPP_INFO(rclcpp::get_logger("Bno08xHardwareInterface"), "Unknown
                    // data_type_code: %d", pData->data_type_code);
                    break;
            }
    }
    ...
    {
        if (elap_since_last_rouge_data_display < elap_since_last_rouge_data_display_span) return;
        char buf[OneLineBuffer_SIZE];
        if (dataSize >= OneLineBuffer_SIZE) dataSize = OneLineBuffer_SIZE - 1;
        int j = 0;
    ...
    hardware_interface::CallbackReturn Bno08xHardwareInterface::get_hardware_parameters(const hardware_interface::HardwareInfo &hardware_info)
    {
        ...
        try       //----------- serial_port_name -------------
        ...
        catch (const std::out_of_range &)
            {
                ...
                RCLCPP_ERROR(rclcpp::get_logger("Bno08xHardwareInterface"), "Bno08xHardwareInterface::get_hardware_parameters() "
                                                                            "'bno08x_serial_port_name' must be defined as a hardware "
        ...
        try       //----------- serial_port_speed -------------
        ...
        catch (const std::out_of_range &)
            {
                ret = CallbackReturn::ERROR;
                RCLCPP_ERROR(rclcpp::get_logger("Bno08xHardwareInterface"), "Bno08xHardwareInterface::get_hardware_parameters() "
                                                                            "'bno08x_serial_port_speed' must be defined as a hardware "
                                                                            "parameter in the URDF file.");
            }
        if (ret != CallbackReturn::SUCCESS) { return ret; }
        ...
        RCLCPP_INFO(rclcpp::get_logger("Bno08xHardwareInterface"),
                    "Bno08xHardwareInterface::get_hardware_parameters() "
                    "bno08x_serial_port_name='%s', bno08x_serial_port_speed=%d,   "
                    "sensor_name='%s'",
                    serial_port_name.c_str(), serial_port_speed, sensor_name.c_str());
