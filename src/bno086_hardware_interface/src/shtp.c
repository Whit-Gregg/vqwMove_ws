...
static void shtpAdvertHdlr(void *cookie, uint8_t tag, [[maybe_unused]] uint8_t len, uint8_t *val)
{
...
static void shtpCmdListener(void *cookie, uint8_t *payload, uint16_t len, [[maybe_unused]] uint32_t timestamp)
{
    shtp_t *pShtp = (shtp_t *)cookie;
